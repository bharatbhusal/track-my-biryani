...
import { findUserByEmail, updateUserPassword, updateUserSettings } from '@/repositories/user.repository';
...
export async function PATCH(request: Request) {
  try {
    ...
    if (payload.password) {
      ...
      const isMatch = await comparePassword(payload.password.currentPassword, user.password);
      if (!isMatch) {
        throw new AppError('Current password is incorrect', 400, 'INVALID_PASSWORD');
      }

      const newHash = await hashPassword(payload.password.newPassword);
