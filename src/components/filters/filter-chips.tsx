...
export function FilterChips({
	...
	if (sections.search && (filterCriteria as any).q) {
		chips.push(
			<Chip
				...
				onRemove={
					actions.setSearch ? () => dispatch(actions.setSearch!(undefined)) : undefined
				}
			/>,
		);
	}
	...
	if (sections.date && !hideDate && datePreset) {
		...
		const clearDate = actions.clearDateFilter ? () => dispatch(actions.clearDateFilter!()) : undefined;
		chips.push(
			<Chip
				key="date"
		...
		if (bucketPreset === "PERSONAL") {
			...
			chips.push(
				<Chip
					key="bucket-all"
					label="All buckets"
					onRemove={actions.clearBucketFilter ? () => dispatch(actions.clearBucketFilter!()) : undefined}
				/>,
			);
			...
			for (const id of bucketIds) {
				...
				chips.push(
					<Chip
						...
						icon={bucket?.icon}
						onRemove={() => removeId(actions.setBucketFilter as never, bucketIds, id, actions.clearBucketFilter)}
					/>,
				);
			}
		}
			...
			for (const id of categoryIds) {
				...
				chips.push(
					<Chip
						...
						icon={category?.emoji}
						onRemove={() => removeId(actions.setCategoryFilter as never, categoryIds, id, actions.clearCategoryFilter)}
					/>,
				);
			}
		}
		...
		if (ownerPreset === "ME") {
			...
			chips.push(
				<Chip
					key="owner-all"
					label="All users"
					onRemove={actions.clearOwnerFilter ? () => dispatch(actions.clearOwnerFilter!()) : undefined}
				/>,
			);
			...
			for (const id of ownerIds) {
				...
				chips.push(
					<Chip
						...
						onRemove={() => removeId(actions.setOwnerFilter as never, ownerIds, id, actions.clearOwnerFilter)}
					/>,
				);
			}
		}
