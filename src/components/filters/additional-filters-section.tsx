      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <label className="space-y-1">
          ...
          <Select
            ...
            onChange={(e) => onChange({ hasNotes: fromValue(e.target.value), hasLocation })}
          >
            {OPTIONS.map((o) => (
        ...
        <label className="space-y-1">
          <span className="text-xs text-[var(--color-muted)]">Has location</span>
          <Select
            value={toValue(hasLocation)}
            onChange={(e) => onChange({ hasNotes, hasLocation: fromValue(e.target.value) })}
          >
            {OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
